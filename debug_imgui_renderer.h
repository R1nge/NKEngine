...
namespace b2 {
    ...
    public:
        ...
        DebugImguiRenderer() {
            ...
            callbacks.DrawSolidPolygonFcn = [](b2Transform xf, const b2Vec2 *vertices, int vertexCount, float radius,
            ...
            callbacks.DrawCircleFcn = [](b2Vec2 center, float radius, b2HexColor color, void *context) {
                auto &self = *static_cast<DebugImguiRenderer *>(context);
                self.DrawList().AddCircle(self.Box2dToImguiPoint(center), self.Box2dToImguiLength(radius),
                                          self.ShapeColorToImguiColor(color, false), 0, self.line_thickness);
            };

            callbacks.DrawSolidCircleFcn = [](b2Transform xf, float radius, b2HexColor color, void *context) {
                auto &self = *static_cast<DebugImguiRenderer *>(context);
                self.DrawList().AddCircleFilled(self.Box2dToImguiPoint(xf.p), self.Box2dToImguiLength(radius),
                                                self.ShapeColorToImguiColor(color, true));
                self.DrawList().AddCircle(self.Box2dToImguiPoint(xf.p), self.Box2dToImguiLength(radius),
                                          self.ShapeColorToImguiColor(color, false), 0, self.line_thickness);
            ...
            callbacks.DrawTransformFcn = [](b2Transform xf, void *context) {
                ...
                self.DrawList().AddLine(ImVec2(p.x - 0.5f, p.y - 0.5f), ImVec2(p.x + d2.x - 0.5f, p.y + d2.y - 0.5f),
                                        ImGui::ColorConvertFloat4ToU32(self.transform_color_y), self.line_thickness);
                self.DrawList().AddLine(ImVec2(p.x - 0.5f, p.y - 0.5f), ImVec2(p.x + d.x - 0.5f, p.y + d.y - 0.5f),
                                        ImGui::ColorConvertFloat4ToU32(self.transform_color_x), self.line_thickness);
            };
            ...
            callbacks.DrawPointFcn = [](b2Vec2 p, float size, b2HexColor color, void *context) {
                ...
                self.DrawList().AddCircleFilled(self.Box2dToImguiPoint(p), size * self.line_thickness,
                                                self.ShapeColorToImguiColor(color, true/*sic, looks better*/));
            };
            ...
            callbacks.DrawStringFcn = [](b2Vec2 p, const char *s, b2HexColor color, void *context) {
                ...
                self.DrawList().AddRectFilled(
                    ...
                    ImVec2(text_pos.x + self.text_background_padding.x + text_size.x,
                           text_pos.y + self.text_background_padding.y + text_size.y),
        ...
        DebugImguiRenderer &operator=(const DebugImguiRenderer &other) {
        ...
        void DrawShapes(b2::WorldRef world) {
            ...
            b2Vec2 half_size(ImGui::GetIO().DisplaySize.x / 2 / camera_scale,
                             ImGui::GetIO().DisplaySize.y / 2 / camera_scale);
        ...
        ImVec2 Box2dToImguiDir(b2Vec2 dir) const {
            ...
            ImVec2 ret(dir.x * camera_rot.c + dir.y * camera_rot.s,
                       (-dir.x * camera_rot.s + dir.y * camera_rot.c) * sign);
            ret.x *= camera_scale;
            ret.y *= camera_scale;
            return ret;
        }
        ...
        b2Vec2 ImguiToBox2dPoint(ImVec2 point) const {
            ...
            float sign = y_axis_is_upwards ? -1 : 1;
            b2Vec2 ret(point.x * camera_rot.c - point.y * camera_rot.s * sign,
                       point.x * camera_rot.s + point.y * camera_rot.c * sign);
            ret.x += camera_pos.x;
            ret.y += camera_pos.y;
            return ret;
        }
    ...
    protected:
        ...
        ImU32 ShapeColorToImguiColor(const b2HexColor &color, bool fill) const {
            return IM_COL32(color >> 16, (color >> 8) & 0xff, color & 0xff,
                            std::clamp(0xff * shape_alpha * (fill ? shape_fill_alpha_factor : 1.f), 0.f, 255.f));
        }
        ...
        void DrawLine(b2Vec2 p1, b2Vec2 p2, b2HexColor color) const {
            ImVec2 a = Box2dToImguiPoint(p1);
            a.x -= 0.5f;
            a.y -= 0.5f; // Counteract the coordinate adjustment in `AddLine()`.
            ImVec2 b = Box2dToImguiPoint(p2);
            b.x -= 0.5f;
            b.y -= 0.5f;
            ...
            if (radius <= 0) {
                ...
                for (int i = 0; i < vertexCount + 2; i++) {
                    ...
                    if (i != 0) {
                        ...
                        if (i != 1) {
                            float f_pi = 3.141592653589793238462643383279502884;
                            if (angle < prev_angle)
                                angle += f_pi * 2;
                            DrawList().PathArcTo(prev_point, Box2dToImguiLength(radius), prev_angle - f_pi / 2,
                                                 angle - f_pi / 2);
                        }
                        prev_angle = angle;
                    }
        ...
        void DrawPolygon(const b2Vec2 *vertices, int vertexCount, b2HexColor color, float radius,
        ...
        void DrawPolygonFilled(const b2Vec2 *vertices, int vertexCount, b2HexColor color, float radius,
                               const b2Transform *xf) const {
            PushPolygonPoints(vertices, vertexCount, radius, xf);
